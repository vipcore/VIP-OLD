...
*Setup instructions for a gitian build of BAC using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BAC
...
Clone the git repositories for vip and gitian and then checkout the vip version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the vip repository
...
To build BAC (for Linux, OSX and Windows) just follow the steps under 'perform
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/vip/.git/
...
./bin/gbuild --commit vip=${COMMIT} --url vip=${URL} ../vip/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/vip-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/vip-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/vip-build.assert
```
...
[vip/gitian.sigs](https://github.com/vip-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
