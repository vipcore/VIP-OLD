...
#ifndef VIP_ZVIPWALLET_H
#define VIP_ZVIPWALLET_H
...
public:
    CzVIPWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZVIP(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZVIP(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //VIP_ZVIPWALLET_H
