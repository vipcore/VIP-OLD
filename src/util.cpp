...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "vip.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        ...
        if (configFile != NULL) {
            ...
            strHeader += "\naddnode=hardseed1.vip.io\naddnode=hardseed2.vip.io\naddnode=hardseed3.vip.io\n";
            strHeader += "txindex=1\nvipstake=1\n";
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override vip.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "vipd.pid"));
