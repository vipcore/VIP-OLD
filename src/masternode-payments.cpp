...
#define DEV_FEE_BLOCK_ACTIVATION 2
...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake, bool fZVIPStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    ...
    if (IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(pindexPrev->nHeight + 1)) {
        ...
        masternodePayments.FillBlockPayee(txNew, nFees, fProofOfStake, fZVIPStake);
    }
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZVIPStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev) return;
	...
	double devfeePercent = pindexPrev->nHeight + 1 >= DEV_FEE_BLOCK_ACTIVATION ? 0.1 : 0.00;
	
    CAmount blockValue = GetBlockValue(pindexPrev->nHeight);
    CAmount masternodePayment = GetMasternodePayment(pindexPrev->nHeight, blockValue, 0, fZVIPStake);
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
	...
	CBitcoinAddress developerfeeaddress("VBkHCDZSf3znuDPtZJck6dmSXhPzzAzF5E");
    CScript developerfeescriptpubkey = GetScriptForDestination(developerfeeaddress.Get());
	double devfeePercent = nBlockHeight >= DEV_FEE_BLOCK_ACTIVATION ? 0.1 : 0.00;
	...
	BOOST_FOREACH (CTxOut out, txNew.vout) {
		if(out.scriptPubKey == developerfeescriptpubkey) {
			if(out.nValue >= requiredDeveloperPayment) {
				...
				LogPrint("masternode", "Developer-Fee Payment found! Thanks for supporting VIP!");
			}
		}
	}
