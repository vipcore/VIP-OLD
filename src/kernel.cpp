...
bool fTestNet = false; //Params().NetworkID() == CBaseChainParams::TESTNET; //not allow to run PoS in TestNet

// Modifier interval: time to elapse before new modifier is computed
// Set to 3-hour for production network and 20-minute for test network
unsigned int nModifierInterval;
int nStakeTargetSpacing = 60; //StakeSpacing 60 seconds
...
{
    //Vip will hash in the transaction hash and the index number in order to make sure each hash is unique
