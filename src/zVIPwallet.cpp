...
CzVIPWallet::CzVIPWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zVIP seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZVIPSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZVIPSeed_deprecated()) {
                    LogPrintf("%s: Updated zVIP seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zVIP seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zVIP wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzVIPWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZVIPCount(nCountLastUsed);
    else if (!walletdb.ReadZVIPCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzVIPWallet::Lock()
{
    seedMaster = 0;
}

void CzVIPWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzVIPWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZVIP(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzVIPWallet::LoadMintPoolFromDB()
{
...
void CzVIPWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzVIPWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzVIPWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zVIPTracker->HasPubcoinHash(pMint.first)) {
...
bool CzVIPWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZVIP(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zVIPTracker which also adds to database
    pwalletMain->zVIPTracker->Add(dMint, true);
...
void CzVIPWallet::SeedToZVIP(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzVIPWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzVIPWallet::GenerateDeterministicZVIP(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzVIPWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzVIPWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
