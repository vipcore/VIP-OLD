...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    ...
    /*
    UniValue zsnoObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zsnoObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zsnoObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.push_back(Pair("zVIPsupply", zsnoObj));
    */
#ifdef ENABLE_WALLET
    if (pwalletMain) {
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "}\n"
            "\nExamples:\n" +
            HelpExampleCli("validateaddress", "\"SffPqR2Dd8Bs1kjXJPmDxDV1aiTNHAeVN2\"") + HelpExampleRpc("validateaddress", "\"1PSSGeFHDnKNxiEyFrD1wcEaHr9hrQDDWc\""));
...
// Vip makekeypair RPC function implemented
...
UniValue getstakingstatus(const UniValue& params, bool fHelp)
{
    ...
    bool nStaking = false;
    printf("mapHashedBlocks.size = %lu\n", mapHashedBlocks.size());
    ...
    if (mapHashedBlocks.count(chainActive.Tip()->nHeight)) {
        ...
        printf("nLastCoinStakeSearchInterval = %lld\n", nLastCoinStakeSearchInterval);
        nStaking = true;
    }
