...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeVipAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeVipAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeVipAmount = strAnonymizeVipAmount.remove(strAnonymizeVipAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeVipAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeVipAmount * COIN) nMaxToAnonymize = nAnonymizeVipAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeVipAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeVipAmount));
        strAnonymizeVipAmount = strAnonymizeVipAmount.remove(strAnonymizeVipAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeVipAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
