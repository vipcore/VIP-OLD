...
#include "ui_zsnocontroldialog.h"
...
std::list<std::string> ZSnoControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZSnoControlDialog::listMints;

ZSnoControlDialog::ZSnoControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZSnoControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZSnoControlDialog::~ZSnoControlDialog()
{
    delete ui;
}

void ZSnoControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSnoControlDialog::updateList()
{
...
void ZSnoControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSnoControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZSno_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSnoControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZSnoControlDialog::GetSelectedMints()
{
...
void ZSnoControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
