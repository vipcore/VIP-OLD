...
void PrivacyDialog::on_pushButtonZSnoControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZSnoControlDialog* zSnoControl = new ZSnoControlDialog(this);
    zSnoControl->setModel(walletModel);
    zSnoControl->exec();
}

void PrivacyDialog::setZSnoControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSnoSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzVIP()
{
    ...
    // use mints from zSno selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZSnoControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZSnoControlDialog::GetSelectedMints();
    }
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zSno was spent successfully update the addressbook with the label
    ...
    // Clear zsno selector in case it was used
    ZSnoControlDialog::listSelectedMints.clear();
    ui->labelzSnoSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zSno Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Sno, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zSno Mint");
