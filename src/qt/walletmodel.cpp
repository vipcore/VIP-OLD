...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered vip address / amount:
...

    {
        ...
        if (recipients[0].useHyperSend && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("HyperSend doesn't support sending values that high yet. Transactions are currently limited to %1 VIP.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal vip:URI (vip:XyZ...?message=example)
            {
...
static void NotifyzVIPReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzVIPReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzVIPReset.connect(boost::bind(NotifyzVIPReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzVIPReset.disconnect(boost::bind(NotifyzVIPReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zVIPTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
